...
1. Each vertex must have a different color than the neighbour vertices (vertices with an edge in common)
2. The number of distinct colors in the entire graph must be as low as possible.
   
It has been proved that this is an NP-complete problem, for which an optimal solution would require a non-polynomial time.
...
2. `Vertex/`: It contains the data structures for the management of the vertex, with representation of the neighbors through adjacency list
...
Template function that allows to asynchronously call a `F&& fn` function for each element of the vector `std::vector<V> &vector`.
...
We have decided to implement the adjacency list using a vector `std::vector<int> _adjV`, since a sequential access is generally made to the neighbors of the vertex.
...
1. `std::vector<Vertex> &getVertices()`: returns the vector of vertices 
...
Functions that allow the generation of an object of type `Graph` by reading a DIMACS (`.gra`) or DIMACS-10 (`.graph`) file.
...
## Solve
All the classes below extend the `Solver` class, which defines an abstract `void solve (Graph &)` method, which is then implemented by the specific subclass solvers.
...
### Jones & Plassmann
`JonesPlassmann` implements the algorithm from [A parallel graph coloring heuristic, M. Jones and T. Plassmann, 1992](https://www.researchgate.net/publication/2768023_A_Parallel_Graph_Coloring_Heuristic). Jones' algorithm defines an ordering of the vertices based on randomly assigned `weights`, and then colors all the nodes having maximum weight among all their uncolored neighbors (*local max*). 

The original Jones' paper describes the algorithm in terms of messages sent between different processors, containing the weights associated with each vertex, previously calculated by the processor.
We generate the `weight` vector sequentially and with random values.
...
Each thread separates its vertices into separator vertices (vertices that have at least one edge connected to a vertex of another processor) and local vertices (the remaining ones).

At each round, the separator vertices that have the greatest weights (`weights[v]` where v represents the vertex) among the neighbors (belonging to the other processors and still not colored) are colored.
...
Once the separator vertices are fully colored, each thread can independently proceed to color the local vertices. In this way the final phase of the algorithm is highly parallelized since no synchronization between the threads is necessary.
...
The algorithm is very similar to the previous JonesPlassmann, but the comparison criterion between the nodes changes: the nodes are colored based on the degree of the vertex. If two adjacent vertices have the same degree, JonesPlassmann's criterion is used.
Comparison criterion:
```cpp
v can be colored
for (auto w: adjV) {    
...
We confirm empirically that LDF and JonesPlassmann are the best solvers in term of total number of color used. 
...
As we can see, the LDF and JonesPlassmann algorithms achieve very similar performances, which reflect and even improve the results obtained by the sequential algorithm. Luby in most cases achieves a higher number of colors than the other algorithms.
...
As can be seen from the graph, the parallel algorithms are extremely more efficient than the sequential one considering a sufficiently high number of threads.
As previously demonstrated, Luby continues to be the worst performing algorithm.
JonesPlassmann and LDF outperform the sequential version.
...
We can see that the memory tends to grow slightly in LDF or JonesPlassmann algorithms, while Luby always occupies almost the same memory with different number of threads.
...
All parallel algorithms scale well with the number of threads and memory usage is not affected considerably.